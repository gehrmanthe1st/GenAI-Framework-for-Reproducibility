...
# 4. validate_data_structure – Verify binary columns are valid (sum to 1 or both 0 for center)
dive_sums <- data$dive_right + data$dive_left
kick_sums <- data$kick_right + data$kick_left

if(any(!dive_sums %in% c(0, 1), na.rm = TRUE)) {
  warning("Some observations in dive_right and dive_left have invalid combinations (not 0 or 1)")
}
if(any(!kick_sums %in% c(0, 1), na.rm = TRUE)) {
  warning("Some observations in kick_right and kick_left have invalid combinations (not 0 or 1)")
}
