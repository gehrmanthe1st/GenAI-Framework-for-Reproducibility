...
for (race in race_groups) {
  ...
  d_WA <- cohen.d(subgroup$WhiteImplicit, subgroup$AsianImplicit, paired = TRUE, pooled = FALSE)
  d_AB <- cohen.d(subgroup$AsianImplicit, subgroup$BlackImplicit, paired = TRUE, pooled = FALSE)
  d_BH <- cohen.d(subgroup$BlackImplicit, subgroup$HispImplicit, paired = TRUE, pooled = FALSE)
...
for (race in race_groups) {
  ...
  d_WA <- effect_sizes[[race]]$d_WA$estimate
  d_AB <- effect_sizes[[race]]$d_AB$estimate
  d_BH <- effect_sizes[[race]]$d_BH$estimate
  combined <- combined %>% 
