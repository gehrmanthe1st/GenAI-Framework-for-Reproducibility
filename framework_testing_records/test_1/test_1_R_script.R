...
play_ci# 8. # Nicer condition analysis - 31.25% chose competent agent
...
# 15. # ---- Additional Analysis: Calculate word frequency for rows where 'attempted' is 'yes' ----
# Assumption: The dataset has columns "word" and "attempted".
# Create a new column 'word_frequency' that gives the frequency of each word among rows with attempted == "yes".
if("word" %in% names(data) & "attempted" %in% names(data)){
  
  # Filter rows where attempted is 'yes'
  attempted_data <- data %>% filter(attempted == "yes")
  
  # Calculate word frequency and create a new column by joining back the table of frequencies
  word_freq_table <- attempted_data %>%
    group_by(word) %>%
    summarise(frequency = n(), .groups = "drop")
  
  attempted_data <- attempted_data %>%
    left_join(word_freq_table, by = "word")
  
  # Print the head of attempted_data with the new 'frequency' column
  print(head(attempted_data))
}
