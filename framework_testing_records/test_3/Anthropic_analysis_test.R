...
required_cols <- c("TrialType", "swt_8.1", "swt_2.1", "RT", "CorrResp", "Participant")
...
overall_descriptives <- data %>%
  ...
  summarise(mean_RT = mean(RT, na.rm = TRUE),
            ...
            .groups = "drop") %>%
  group_by(TrialType) %>%
  summarise(overall_mean_RT = mean(mean_RT, na.rm = TRUE),
            overall_accuracy = mean(accuracy, na.rm = TRUE),
            .groups = "drop")

print("Overall descriptive measures:")
...
participant_means <- full_join(predictive_summary, nonpredictive_summary, by = "Participant")
...
switch_cost <- overall_descriptives %>%
  filter(TrialType %in% c(1, 2)) %>%
  summarise(RT_switch_cost = overall_mean_RT[TrialType == 2] - overall_mean_RT[TrialType == 1],
            accuracy_switch_cost = overall_accuracy[TrialType == 2] - overall_accuracy[TrialType == 1])
